...
const {DNC20TokenBankContract, DNC721TokenBankContract, DNC1155TokenBankContract, TokenType, DontCare, 
...
const {DNC20ABI, DNC721ABI} = require("../../constants/contracts");
...
const MockDNC20Contract = {abi: DNC20ABI};
const MockDNC721Contract = {abi: DNC721ABI};
const MockDNC1155Contract = null;

const Wallet = require("../Wallet").default;



// Note: a voucher is also a DNC token itself, so we can use the following function to retrieve
//       the voucher balance of an address
async function getDNCTokenBalance(tokenType, chainIDStr, rpc, tokenContractAddress, tokenID, queriedAddress) {
    ...
    if (tokenType === TokenType.DNC20) {
        const dnc20Contract = new dnerojs.Contract(tokenContractAddress, MockDNC20Contract.abi, provider);
        tokenBalance = await dnc20Contract.balanceOf(queriedAddress);
    } else if (tokenType === TokenType.DNC721) {
        const dnc721Contract = new dnerojs.Contract(tokenContractAddress, MockDNC721Contract.abi, provider);
        try {
            tokenOwner = await dnc721Contract.ownerOf(tokenID);
        } catch { // DNC721 contract asserts if the NFT with tokenID has already burned
            tokenOwner = 0x0;
        }
        tokenBalance = (tokenOwner === queriedAddress) ? 1 : 0;
    } else if (tokenType === TokenType.DNC1155) {
        const dnc1155Contract = new dnerojs.Contract(tokenContractAddress, MockDNC1155Contract.abi, provider);
        tokenBalance = await dnc1155Contract.balanceOf(queriedAddress, tokenID);
    } else {
        throw new Error ("getDNCTokenBalance: Invalid tokenType")
    }

    return tokenBalance;
}

// Note: a voucher is also a DNC token itself, so we can use the following function to approve
//       voucher access
async function approveTokenAccess(tokenType, from, sourceChainIDStr, sourceChainRPC, tokenContractAddress, spenderAddr, tokenID, amountInWei) {
    ...
    let approveTx;
    if (tokenType === TokenType.DNC20) {
        const dnc20Contract = new dnerojs.Contract(tokenContractAddress, MockDNC20Contract.abi, null);
        approveTx = await dnc20Contract.populateTransaction.approve(spenderAddr, amountInWei);
    } else if (tokenType === TokenType.DNC721) {
        const dnc721Contract = new dnerojs.Contract(tokenContractAddress, MockDNC721Contract.abi, null);
        approveTx = await dnc721Contract.populateTransaction.approve(spenderAddr, tokenID);
    } else if (tokenType === TokenType.DNC1155) {
        const dnc1155Contract = new dnerojs.Contract(tokenContractAddress, MockDNC1155Contract.abi, null);
        approveTx = await dnc1155Contract.populateTransaction.setApprovalForAll(spenderAddr, true);
    } else {
        throw new Error("getDNCTokenBalance: Invalid tokenType")
    }
...
async function isVoucherContract(tokenType, sourceChainIDStr, sourceChainRPC, tokenOrVoucherContractAddr) {
    ...
    if (sourceChainIDStr === cfg().mainchainIDStr) {
        if (tokenType === TokenType.DNC20) {
            tokenBankContract = new dnerojs.Contract(cfg().mainchainDNC20TokenBankAddr, DNC20TokenBankContract.abi, provider);
        } else if (tokenType === TokenType.DNC721) {
            tokenBankContract = new dnerojs.Contract(cfg().mainchainDNC721TokenBankAddr, DNC721TokenBankContract.abi, provider);
        } else if (tokenType === TokenType.DNC1155) {
            tokenBankContract = new dnerojs.Contract(cfg().mainchainDNC1155TokenBankAddr, DNC1155TokenBankContract.abi, provider);
        } else {
            throw new Error("isVoucherContract: Invalid tokenType (mainchain)")
        }
    } else if (sourceChainIDStr === cfg().subchainIDStr) {
        if (tokenType === TokenType.DNC20) {
            tokenBankContract = new dnerojs.Contract(cfg().subchainDNC20TokenBankAddr, DNC20TokenBankContract.abi, provider);
        } else if (tokenType === TokenType.DNC721) {
            tokenBankContract = new dnerojs.Contract(cfg().subchainDNC721TokenBankAddr, DNC721TokenBankContract.abi, provider);
        } else if (tokenType === TokenType.DNC1155) {
            tokenBankContract = new dnerojs.Contract(cfg().subchainDNC1155TokenBankAddr, DNC1155TokenBankContract.abi, provider);
        } else {
...
function getSourceAndTargetChainTokenBank(tokenType, targetChainID) {
    ...
    if (tokenType === TokenType.DNC20) {
        mainchainTokenBankContract = new dnerojs.Contract(cfg().mainchainDNC20TokenBankAddr, DNC20TokenBankContract.abi, mainchainProvider);
        subchainTokenBankContract = new dnerojs.Contract(cfg().subchainDNC20TokenBankAddr, DNC20TokenBankContract.abi, subchainProvider);
    } else if (tokenType === TokenType.DNC721) {
        mainchainTokenBankContract = new dnerojs.Contract(cfg().mainchainDNC721TokenBankAddr, DNC721TokenBankContract.abi, mainchainProvider);
        subchainTokenBankContract = new dnerojs.Contract(cfg().subchainDNC721TokenBankAddr, DNC721TokenBankContract.abi, subchainProvider);
    } else if (tokenType === TokenType.DNC1155) {
        mainchainTokenBankContract = new dnerojs.Contract(cfg().mainchainDNC1155TokenBankAddr, DNC1155TokenBankContract.abi, mainchainProvider);
        subchainTokenBankContract = new dnerojs.Contract(cfg().subchainDNC1155TokenBankAddr, DNC1155TokenBankContract.abi, subchainProvider);
    } else {
...
async function lockDNCTokens(tokenType, from, sourceChainTokenAddr, targetChainID, targetChainReceiver, tokenID, amountInWei) {
    const keyringController = Wallet.controller.keyringController;
    const transactionController = Wallet.controller.transactionsController;
    const signAndSendTransaction = keyringController.signAndSendTransaction.bind(keyringController);
    const provider = createProvider(cfg().mainchainIDStr, cfg().mainchainRPC);

    let value = expandTo18Decimals(cfg().crossChainTransferFeeInDToken);
    let tx;
    ...
    if (tokenType === TokenType.DNC20) {
        ...
        throw new Error("lockDNCTokens: Invalid tokenType")
    }
...
async function burnDNCVouchers(tokenType, from, sourceChainVoucherAddr, denom, targetChainReceiver, tokenID, amountInWei) {
    const keyringController = Wallet.controller.keyringController;
    const transactionController = Wallet.controller.transactionsController;
    const signAndSendTransaction = keyringController.signAndSendTransaction.bind(keyringController);
    ...
    if (tokenType === TokenType.DNC20) {
        ...
        throw new Error("burnDNCVouchers: Invalid tokenType")
    }
...
async function transferDNC(tokenType, from, sourceChainTokenOrVoucherContractAddr, targetChainID, targetChainReceiver, tokenID, amountInWei) {
    if (targetChainID !== cfg().subchainID && targetChainID !== cfg().mainchainID) {
        throw new Error("transferDNC: Invalid target chain ID");
    }
    ...
    if (isVoucher) { // sourceChainTokenOrVoucherContractAddr is a voucher contract address
        ...
        targetChainTokenOrVoucherContractAddr = await targetChainTokenBankContract.getVoucher(denom); // should return 0x0 if the voucher contract has not been deployed yet, e.g. before the first transfer of the DNC token
    }
    ...
    await printSenderReceiverBalances(tokenType,
        ...
        targetChainIDStr, targetChainRPC, targetChainTokenOrVoucherContractAddr, tokenID, targetChainReceiver, getDNCTokenBalance);

    let targetChainReceiverInitialBalance = await getDNCTokenBalance(tokenType, targetChainIDStr, targetChainRPC, targetChainTokenOrVoucherContractAddr, tokenID, targetChainReceiver);
    ...
    if (!isVoucher) { // sourceChainTokenOrVoucherContractAddr is a token contract address
        let lockTx = await lockDNCTokens(tokenType, from, sourceChainTokenOrVoucherContractAddr, targetChainID, targetChainReceiver, tokenID, amountInWei);
        ...
        let burnTx = await burnDNCVouchers(tokenType, from, sourceChainTokenOrVoucherContractAddr, denom, targetChainReceiver, tokenID, amountInWei);
    ...
    await detectTargetChainReceiverBalanceChanges(targetChainIDStr, targetChainRPC, tokenType, denom, 
        targetChainTokenBankContract, targetChainTokenOrVoucherContractAddr, tokenID, targetChainReceiver, targetChainReceiverInitialBalance, getDNCTokenBalance);
    ...
    if(tokenType === TokenType.DNC20 && targetChainID !== cfg().mainchainID){
        // Get the token data...
        const {dneroWallet} = store.getState();
        const {tokens} = dneroWallet;
        ...
        console.log('transferDNC :: TRACKING TOKEN...');
        ...
        console.log('transferDNC :: NOT TRACKING TOKEN BECAUSE IT IS SENDING TO MAINCHAIN!');
    }
